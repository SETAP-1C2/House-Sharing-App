...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const groupDescription = localStorage.getItem("groupDescription");
  const groupId = localStorage.getItem("groupId");

  const nameDisplay = document.querySelector("#group-name-display");
  const descDisplay = document.querySelector("#group-description-display");
  const idDisplay = document.querySelector("#group-id-display");

  if (nameDisplay) nameDisplay.textContent = groupName || "Unnamed Group";
  if (descDisplay) descDisplay.textContent = groupDescription || "No description provided.";
  if (idDisplay) idDisplay.textContent = groupId || "Unknown ID";
});
